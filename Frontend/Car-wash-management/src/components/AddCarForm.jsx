...
export function AddCarForm() {
  ...
  async function handleSubmit() {
    ...
    if (!form.customerName.trim() || !form.phoneNumber.trim()) {
      setMessage({
        type: "error",
        text: "Customer name and phone number are required.",
      });
      return;
    }

    // Strip non-digits and validate length
    let cleanPhone = form.phoneNumber.replace(/\D/g, "");
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `${car.customerName}'s car added! Notification sent. ✅`,
      });
      ...
      setMessage({
        type: "warning",
        text: "Failed to add car. Check if backend is running.",
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="form-row">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const initCap = val.replace(/\b\w/g, (c) => c.toUpperCase());
              update("customerName", initCap);
            }}
          />
        </div>
        <div className="form-row">
